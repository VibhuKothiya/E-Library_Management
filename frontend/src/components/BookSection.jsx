...
const BookSection = ({ searchQuery }) => {
    ...
    const getBooks = async () => {
        let result = await axios.get('http://localhost:3050/library/books/getBook')
    ...
    const onDelete = async (_id) => {
        try {
            await axios.delete(`http://localhost:3050/library/books/deleteBook/${_id}`);
    ...
    const handleUpdate = async () => {
        try {
            const res = await axios.put(`http://localhost:3050/library/books/updateBook/${view._id}`, view);
